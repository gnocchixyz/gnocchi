...
from gnocchi.openstack.common.gettextutils import _LE, _LW
    ...
    def _do_query(self, q):
        ...
        for filename, line, method, function in traceback.extract_stack():
            ...
            if filename.endswith('db/api.py'):
                continue
            # only trace inside gnocchi
            index = filename.rfind('gnocchi')
            if index == -1:
                continue
...
class EngineFacade(object):
    """A helper class for removing of global engine instances from gnocchi.db.

    As a library, gnocchi.db can't decide where to store/when to create engine
    and sessionmaker instances, so this must be left for a target application.

    On the other hand, in order to simplify the adoption of gnocchi.db changes,
